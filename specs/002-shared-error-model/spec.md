...
- **FR-002**: The system MUST support exactly three severity levels: "warning" (non-critical issue, processing continues), "error" (file-level failure, processing continues for other files), and "fatal" (process-level failure, processing stops after recording the error).
...
- **FR-004**: The system MUST record all errors — from all pipeline stages and all severity levels — in a single `error.log` file located in the target directory. If the target directory does not exist or is inaccessible, the system MUST fall back to writing `error.log` in the current working directory (CWD) where the user invoked the tool.
...
- **FR-007**: The system MUST flush each error entry to disk immediately (or at minimum after each file is processed), so that errors are persisted even if the process is interrupted.
...
- **FR-015**: The system MUST support tagging errors with additional context: for "warning" severity, the entry SHOULD include the reason the item was skipped; for "error" severity, the entry MUST include the filename and the failure reason; for "fatal" severity, the entry MUST include the failure reason and the state at which processing stopped.
- **FR-016**: Error entries MUST be written in a way that a partially written entry (e.g., due to crash during write) does not corrupt previously written entries. Each entry MUST be self-contained on its own line(s).
...
- **Error Log**: The persistent, append-only `error.log` file. Primary location: the target directory. Fallback location: the current working directory (CWD) if the target directory is invalid. Accumulates error entries across multiple runs, separated by run markers. Serves as the single source of truth for all errors produced by the tool. Written via single-threaded serial access.
...
- The `error.log` file is located in the target directory (the directory being processed), consistent with spec 001. If the target directory is invalid (does not exist or is inaccessible), `error.log` falls back to the current working directory (CWD).
