...
1. **Given** a directory with 20 form images, **When** the OCR reader processes them, **Then** it uses a `TaskGroup` with a maximum of 4–8 concurrent tasks (per Constitution Principle III).
...
1. **Given** a form with a printed header, **When** OCR processes it, **Then** the `FormData.header` property contains the recognized header text lines.
2. **Given** a form without a recognizable header, **When** OCR processes it, **Then** `FormData.header` is empty and no error is thrown.

---

### Edge Cases

- What happens when handwriting overlaps printed text? → The OCR reader should extract the best-confidence text and include it; accuracy limitations are acceptable for a POC.
- What happens with very poor handwriting that Vision cannot recognize? → The field is treated as empty and excluded (per Story 2 rules). A per-image confidence score is tracked.
- What happens when a label spans multiple printed lines (e.g., "Fecha y lugar de nacimiento:")? → The full multi-word label should be captured as a single key.
- What happens with Spanish special characters (ñ, á, é, í, ó, ú, ü)? → Vision framework with `.accurate` recognition and Spanish language hint must handle them. If not recognized, the raw best-guess text is preserved.
- What happens when the image is rotated or skewed? → Rely on Vision's built-in image orientation handling; no custom deskew logic for POC.
...
- **FR-003**: For each recognized text observation (`VNRecognizedTextObservation`), the system MUST capture: the recognized string, its confidence score, and its bounding box coordinates.
...
- **NFR-006**: Vision request processing MUST NOT block the main thread; all work is dispatched via structured concurrency.
